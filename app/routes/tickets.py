...
# --- UTILITAIRES ---
def nocache(view):
    """Décorateur anti-cache renforcé pour les pages tickets"""
    @wraps(view)
...
def check_permission(required_roles):
    # Conversion en string majuscule
    try:
...
def new_ticket(service_name):
    ...
    if request.method == 'POST':
        ...
        # --- DÉTERMINATION DU STATUT INITIAL (WORKFLOW) ---
        role = current_user.role
        
        # Exception : Incident Standard Informatique => Pas de validation
        if service_enum == ServiceType.INFO and category == "Incident Standard":
            status = TicketStatus.PENDING
        else:
            if role == UserRole.USER:
                status = TicketStatus.VALIDATION_N1 # Vers Manager
            elif role == UserRole.MANAGER:
                status = TicketStatus.VALIDATION_N1 # Vers Directeur
            elif role == UserRole.DIRECTEUR:
                status = TicketStatus.VALIDATION_N2 # Directeur auto-valide N1 -> Vers Service Cible (N2)
        ...
        while not saved and attempts < 5:
            ...
            t = Ticket(
                ...
                service_demandeur=selected_origin, # Le GU choisi
        ...
        if not saved:
            flash("Erreur lors de la création (Conflit ID).", "danger")
            return redirect(url_for('tickets.new_ticket', service_name=service_name))
        ...
        if status == TicketStatus.VALIDATION_N1:
            ...
            for mgr in managers:
                # Vérifie si le manager gère le service d'origine du ticket
                if selected_origin in mgr.get_origin_services():
                    create_notification(mgr, f"Validation N1 requise : {uid} ({selected_origin})", 'warning', link_mgr)
            ...
            for tm in target_managers:
                # Vérifie si le manager gère le service cible (GS)
                if service_name in tm.get_allowed_services(): 
                    create_notification(tm, f"Validation N2 requise : {uid}", 'warning', link_mgr)
        ...
        flash(f'Demande {uid} enregistrée (Statut: {status.value}).', 'success')
        return redirect(url_for('main.user_portal'))
...
def manager_dashboard():
    try:
        # Ici, on AUTORISE le DIRECTEUR explicitement
        if not check_permission(['MANAGER', 'DIRECTEUR', 'ADMIN']):
            # Si c'est un Solver qui essaie d'accéder ici par le bouton "Retour", on le renvoie chez lui
            if 'SOLVER' in str(current_user.role).upper():
        ...
        # 1. À VALIDER N1 (Demandes de mes équipes)
        tickets_n1 = []
        if my_origins: 
        ...
        # 2. À VALIDER N2 (Demandes pour mon service technique)
        tickets_n2 = []
        if my_targets:
            # Récupération brute puis filtrage Python car target_service est Enum en DB et my_targets est liste de Strings
        ...
        print(f"ERREUR FATALE MANAGER DASHBOARD: {e}")
        import traceback
        traceback.print_exc()
        flash(f"Erreur serveur (500) : {e}", "danger")
        return render_template('base.html', content=f"<h1>Erreur Serveur</h1><p>{e}</p>")
...
def manager_action(ticket_id, action):
    try:
        ...
        # CORRECTION : Suppression de l'ancienne vérification basée sur service_department
        
        if action == 'validate':
            if t.status == TicketStatus.VALIDATION_N1:
                # N1 validé -> Passe en N2 (Service Cible)
                t.status = TicketStatus.VALIDATION_N2
                flash(f"Ticket {t.uid_public} validé hiérarchiquement. Envoyé au service destinataire.", "success")
                ...
                for tm in target_managers:
                    ...
                    if t.target_service.value in tm.get_allowed_services():
                        create_notification(tm, f"Validation N2 requise : {t.uid_public}", 'warning', url_for('tickets.manager_dashboard'))

            elif t.status == TicketStatus.VALIDATION_N2:
                # N2 validé -> Passe en PENDING (Pool Technicien)
                t.status = TicketStatus.PENDING
                flash(f"Ticket {t.uid_public} validé techniquement. En attente de traitement.", "success")
                ...
                for s in solvers:
                    ...
                    if s.role == UserRole.ADMIN or t.target_service.value in s.get_allowed_services():
                        create_notification(s, f"Nouveau ticket validé : {t.uid_public}", 'info', url_for('tickets.solver_dashboard'))
        ...
        print(f"ERREUR MANAGER ACTION: {e}")
...
def solver_dashboard():
    try:
        # STRICT : Si pas le droit, Catdance
        if not check_permission(['SOLVER', 'ADMIN']):
            return render_template('errors/catdance.html'), 403

        # CORRECTION : Utilisation de la nouvelle méthode get_allowed_services()
        ...
        # Filtrage par service (sauf Admin)
        if 'ADMIN' not in user_role and my_targets:
            tickets_all = query.all()
            # Filtrage Python pour vérifier si le service du ticket est dans la liste des services autorisés
        ...
        pool_standard = [t for t in pending_tickets if not t.category_ticket or t.category_ticket == 'Standard' or t.category_ticket == 'Incident Standard']
        ...
        # On récupère tous les solvers pour l'assignation
        ...
        print(f"ERREUR FATALE SOLVER DASHBOARD: {e}")
        import traceback
        traceback.print_exc()
        flash(f"Erreur serveur (500) : {e}", "danger")
        return render_template('base.html', content=f"<h1>Erreur Serveur</h1><p>{e}</p>")
...
def view_ticket(ticket_uid):
    ...
    can_view = False
    # CORRECTION : Utilisation des nouvelles méthodes pour vérifier les droits
...
def take_ticket(ticket_id):
    if not check_permission(['SOLVER', 'ADMIN']): return render_template('errors/catdance.html'), 403
    t = Ticket.query.get_or_404(ticket_id)
...
def historique_tickets():
    # CORRECTION : Suppression de la référence à service_department ici aussi
    my_targets = current_user.get_allowed_services()
    user_role = str(current_user.role.value).upper() if hasattr(current_user.role, 'value') else str(current_user.role).upper()
    ...
    if request.method == 'POST':
        ...
        # Pour le ticket manuel, on prend le premier service autorisé du solver, ou INFO par défaut
        target_svc = ServiceType.INFO
        if my_targets:
            try: target_svc = ServiceType[my_targets[0]]
            except: pass

        t = Ticket(title=title, description=f"Ticket manuel pour : {user_name}\n\n{description}", author=current_user, target_service=target_svc, status=TicketStatus.DONE, uid_public=uid, created_at=d_create, closed_at=datetime.now(), solver=current_user)
        db.session.add(t)
        db.session.commit()
